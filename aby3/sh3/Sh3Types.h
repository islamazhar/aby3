...
namespace aby3
{
        ...
        struct si64; /// what this line is doing?
        struct si32;
    ...
    ////// a shared 32 bit integer. Added by Mazharul
    struct si32
    {
        ...
        si32() = default;
        si32(const si32&) = default;
        si32(si32&&) = default;
        si32(const std::array<value_type, 2>& d) :mData(d) {}
        si32(const Ref<si32>& s) {
            mData[0] = *s.mData[0];
            mData[1] = *s.mData[1];
        }

        si32& operator=(const si32& copy); // where = is declared?
        si32 operator+(const si32& rhs) const;
        si32 operator-(const si32& rhs) const;
    ...
    // a shared 32 binary value
    struct sb32
    {
        using value_type = i32;
        std::array<i32, 2> mData;

        sb32() = default;
        sb32(const sb32&) = default;
        sb32(sb32&&) = default;
        sb32(const std::array<value_type, 2>& d) :mData(d) {}

        i32& operator[](u64 i) { return mData[i]; }
        const i32& operator[](u64 i) const { return mData[i]; }


        sb32& operator=(const sb32& copy) = default;
        sb32 operator^(const sb32& x) { return { { mData[0] ^ x.mData[0], mData[1] ^ x.mData[1] } }; }
        ...
        struct si32Matrix
        {
            std::array<eMatrix<i32>, 2> mShares;

            struct ConstRow { const si32Matrix& mMtx; const u64 mIdx; };
            struct Row { si32Matrix& mMtx; const u64 mIdx; const Row& operator=(const Row& row); const ConstRow& operator=(const ConstRow& row); };

            struct ConstCol { const si32Matrix& mMtx; const u64 mIdx; };
            struct Col { si32Matrix& mMtx; const u64 mIdx; const Col& operator=(const Col& col); const ConstCol& operator=(const ConstCol& row); };

            si32Matrix() = default;
            si32Matrix(u64 xSize, u64 ySize)
            {
                resize(xSize, ySize);
            }
            ...
            Ref<si32> operator()(u64 x, u64 y) const;
            Ref<si32> operator()(u64 xy) const;
            si32Matrix operator+(const si32Matrix& B) const;
            si32Matrix operator-(const si32Matrix& B) const;

            si32Matrix transpose() const;
            void transposeInPlace();
            ...
            bool operator !=(const si32Matrix& b) const
            {
                return !(*this == b);
            }
...
//        }
    ...
    inline si32& si32::operator=(const si32& copy)
    {
        mData[0] = copy.mData[0];
        mData[1] = copy.mData[1];
        return *this;
    ...
    inline si32 si32::operator+(const si32& rhs) const
    {
        si32 ret;
        ret.mData[0] = mData[0] + rhs.mData[0];
        ret.mData[1] = mData[1] + rhs.mData[1];
    ...
    inline Ref<si32> si32Matrix::operator()(u64 x, u64 y) const
    {
        return Ref<si32>(
                (i32&)mShares[0](x, y),
                (i32&)mShares[1](x, y));
    ...
    inline Ref<si32> si32Matrix::operator()(u64 xy) const
    {
        return Ref<si32>(
                (i32&)mShares[0](xy),
                (i32&)mShares[1](xy));
    ...
    inline si32Matrix si32Matrix::operator+(const si32Matrix& B) const
    {
        si32Matrix ret;
        ret.mShares[0] = mShares[0] + B.mShares[0];
        ret.mShares[1] = mShares[1] + B.mShares[1];
    ...
    inline si32Matrix si32Matrix::transpose() const
    {
        si32Matrix ret;
        ret.mShares[0] = mShares[0].transpose();
        ret.mShares[1] = mShares[1].transpose();
    ...
    inline void si32Matrix::transposeInPlace()
    {
        mShares[0].transposeInPlace();
        mShares[1].transposeInPlace();
    }

    inline si32Matrix::Row si32Matrix::row(u64 i)
    {
        return Row{ *this, i };
    }
    inline si32Matrix::ConstRow si32Matrix::row(u64 i) const
    {
        return ConstRow{ *this, i };
    }
    inline si32Matrix::Col si32Matrix::col(u64 i)
    {
        return Col{ *this, i };
    }
    inline si32Matrix::ConstCol si32Matrix::col(u64 i) const
    {
        return ConstCol{ *this, i };
    }

    inline const si32Matrix::Row & si32Matrix::Row::operator=(const Row & row)
    {
        mMtx.mShares[0].row(mIdx) = row.mMtx.mShares[0].row(row.mIdx);
        mMtx.mShares[1].row(mIdx) = row.mMtx.mShares[1].row(row.mIdx);
    ...
    inline const si32Matrix::ConstRow & si32Matrix::Row::operator=(const ConstRow & row)
    {
        mMtx.mShares[0].row(mIdx) = row.mMtx.mShares[0].row(row.mIdx);
        mMtx.mShares[1].row(mIdx) = row.mMtx.mShares[1].row(row.mIdx);
        return row;
    ...
    inline const si32Matrix::Col & si32Matrix::Col::operator=(const Col & col)
    {
        mMtx.mShares[0].col(mIdx) = col.mMtx.mShares[0].col(col.mIdx);
        mMtx.mShares[1].col(mIdx) = col.mMtx.mShares[1].col(col.mIdx);
        return col;
    ...
    inline const si32Matrix::ConstCol & si32Matrix::Col::operator=(const ConstCol & col)
    {
        mMtx.mShares[0].col(mIdx) = col.mMtx.mShares[0].col(col.mIdx);
        mMtx.mShares[1].col(mIdx) = col.mMtx.mShares[1].col(col.mIdx);
        return col;
